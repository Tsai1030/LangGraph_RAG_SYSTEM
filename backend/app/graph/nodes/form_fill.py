...
class _Extraction(BaseModel):
    ...
    user_done: bool = Field(
        default=False,
        description="使用者是否表達結束填表（『就這樣』『改完了』『OK』）",
    )
...
def _decide_status(
    ...
    - 有批次編輯：強制 collecting（等使用者再說『就這樣』確認，避免 LLM 把『改成 X』誤判 user_done=True）
...
4. user_done / auto_fill_test
   - user_done=true 限定**結束指令**：「就這樣」「都填好了」「OK」「完成」「改完了」「改好了」
...
async def form_fill_collector(state: GraphState) -> dict:
    ...
    new_session: dict = {**session, "collected": collected, "status": status}
    if bulk_summary:
    ...
    logger.info(
        "[form_fill_collector] form=%s extracted=%d ghost=%d bulk=%d auto=%d collected=%d "
        "user_done=%s auto_test=%s → status=%s",
        target_id, len(extracted_keys), len(ghost_keys), n_bulk, n_auto, len(collected),
        extraction.user_done, extraction.auto_fill_test, status,
    )
    return {"form_fill_session": new_session}
